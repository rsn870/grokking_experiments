...
class AbstractDataset_Double(abc.ABC):
    """

    A two operation at a time abstract dataset with the same inputs but  modified fetch_output and fetch_example functions

    Appropriately changes have been made in a copy of train_grokk.py 
    ...
    def fetch_example(self, idx):
        a = self.ordered_group_elements1[idx // len(self.group_elements2)]
        b = self.ordered_group_elements2[idx % len(self.group_elements2)]
        c1,c2 = self.fetch_output(a, b)
        equation1 = self.form_equation(a, b, c1)
        equation2 = self.form_equation(a, b, c2)
        enc1 = (self.encode(equation1[:-1]), (self.vocab2idx[c1]-2), equation1)
        enc2 = (self.encode(equation2[:-1]), (self.vocab2idx[c2]-2), equation2)  #Converting into tuples
        return enc1,enc2
    ...
    class ModSumSubtractDataset(AbstractDataset_Double):
        def __init__(self, p, frac_train):
            super(ModSumSubtractDataset, self).__init__(set(range(p)), set(range(p)), frac_train)
            self.p = p
        
        def fetch_output(self, a, b):
            return ((a + b) % self.p, (a - b) % self.p)
        
    class ModSumDivisonDataset(AbstractDataset_Double):
        def __init__(self, p, frac_train):
            super(ModSumDivisonDataset, self).__init__(set(range(p)), set(range(1, p)), frac_train)
            self.p = p
        
        def fetch_output(self, a, b):
            return ((a + b) % self.p, (a * pow(b, self.p-2, self.p)) % self.p)
        
    class ModSubtractDivisonDataset(AbstractDataset_Double):
        def __init__(self, p, frac_train):
            super(ModSubtractDivisonDataset, self).__init__(set(range(p)), set(range(1, p)), frac_train)
            self.p = p
